...
| [records:{object_api_slug}](https://docs.attio.com/rest-api/endpoint-reference/records/list-records) | - | - | replace | Fetches all records of an object. For example: `records:companies`. Full reload on each run. An alternative query-parameter form is also accepted: `records?object=companies`. |
...
| [list_entries:{list_id}](https://docs.attio.com/rest-api/endpoint-reference/entries/list-entries) | - | - | replace | Lists all items in a specific list. For example: `list_entries:8abc-123-456-789d-123`. Full reload on each run. An alternative query-parameter form is also accepted: `list_entries?list_id=8abc-123-456-789d-123`. |
